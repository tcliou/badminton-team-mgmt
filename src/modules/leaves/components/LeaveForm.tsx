...
import { useCreateLeave } from '../api/leavesApi';
...
export function LeaveForm({ onSuccess }: LeaveFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-xl border bg-card p-4">
      ...
      <Button type="submit" disabled={isSubmitting || createLeave.isPending} className="w-full sm:w-auto">
        {isSubmitting || createLeave.isPending ? t('common.loading') : t('leaves:form.submit')}
      </Button>
    </form>
  );
}
